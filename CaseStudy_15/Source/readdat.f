**==readdat.spg  processed by SPAG 4.52O  at 10:35 on 12 Jul 1996
      SUBROUTINE READDAT(Equil, Prod, Nsamp, Nghost, Ndispl, Dr)
...
c    Nghost     : number of attempts to insert a particle
...
c     eps, sig, MASS, RC : parameters potential
c
...
c    X(NPART),Y(NPART),Z(NPART): position particle last particle
      ...
      INCLUDE 'conf.inc'
      INTEGER ibeg, Equil, Prod, i, Ndispl, Nsamp, Nghost
      DOUBLE PRECISION eps, sig, CORU, CORP, vir, boxf, rhof, rho, Dr
 
 
c     ---read simulation data
      READ (15, *)
      READ (15, *) ibeg, Equil, Prod, Nsamp, Nghost
      READ (15, *)
      READ (15, *) Dr
      READ (15, *)
      READ (15, *) Ndispl
...
c        ---generate configuration form lattice
      ...
      END IF
c     ---write input data
      WRITE (6, 99001) Equil, Prod, Nsamp, Nghost
...
99001 FORMAT ('  Number of equilibration cycles             :', i10, /, 
     ...
     &        '  Number of test part. insertion per sample  :', i10, /)
