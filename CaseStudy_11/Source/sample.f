**==sample.spg  processed by SPAG 4.52O  at 11:24 on 20 Jun 1996
      SUBROUTINE SAMPLE(Switch, Is, Time, En, Vir, Enk, Delt, H)
c     writes quantities to file
     ...
     &        tvacf, ngr, t0, dt, ifail
      PARAMETER (NHIsmax=250, TMAx=5000, T0Max=100)
      INTEGER nvacf(TMAx), tstress0, tstress, nstress(TMAx), tt0(T0Max), 
     &        ttel, ttv0(T0Max)
      DOUBLE PRECISION En, enp, Vir, press, vol, rho, Enk, temp, Time
      DOUBLE PRECISION tempav, delg, delgi, gr(NHIsmax), r, e, p, r6i, 
     &                 r2, dx, dy, dz, xi, yi, zi, enk2, dr, Delt, CORP
     ...
     &                 rz0(NPMax, T0Max), ri(NHIsmax), ans, err
      DOUBLE PRECISION sxyt(TMAx), sxzt(TMAx), syzt(TMAx), sxy0(T0Max), 
     &                 sxz0(T0Max), syz0(T0Max), sxy, sxz, syz, H, 
     ...
     &        binvel2(NHIsmax)
      SAVE ngr, tempav, delg, delgi, gr, nhgr
...
c        ---radial distribution function
         IF (ngr.NE.0) THEN
            ...
            DO i = 1, nhgr
               ...
               gr(i) = 2*gr(i)/(ngr*dr*rho*NPART)
               WRITE (IOUT1, *) r, gr(i)
               IF (r*r.LT.RC2) THEN
                  r6i = 1/r**6
                  e = e + gr(i)*2*PI*rho*r**2*(4*(r6i*r6i-r6i)-ECUT)
     &                *delg
                  p = p + gr(i)*2*PI*rho**2*r**2*48*(r6i*r6i-0.5D0*r6i)
     &                *delg/3
                  gr(i) = gr(i)*2*PI*rho*r**2*(4*(r6i*r6i-r6i)-ECUT)
               ELSE
                  gr(i) = 0
               END IF
               ri(i) = i*delg
            END DO
            ifail = 0
c            CALL D01GAF(ri, g, nhgr, ans, err, ifail)
            PRINT *, ' energy ', SNGL(ans), ' +/- ', SNGL(err)
...
c           ---temp fluc
            ...
            DO i = 1, NHIsmax
               IF (bintemp(i).NE.0) THEN
                  WRITE (IOUT3+1, '(2(2x,f8.3))') (i+0.5)/factemp, 
     &                   (bintemp(i)/DBLE(itt))/factemp
               END IF
               IF (binvel(i).NE.0) THEN
                  WRITE (IOUT3+2, '(2(2x,f8.3))') (i+0.5)/factvel - v0, 
     &                   sum*(binvel(i)/DBLE(NPART*itt*3))
               END IF
               IF (binvel2(i).NE.0) THEN
                  WRITE (IOUT3+4, '(2(2x,f10.3))') (i-0.5)/factvel2, 
