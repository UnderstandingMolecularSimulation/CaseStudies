**==init.spg  processed by SPAG 4.52O  at 11:24 on 20 Jun 1996
...
c        ---generate configuration form lattice
         ...
         HBOX = 0.5D00*BOX
         CALL LATTICE()
         CALL SETVEL(temp, Iseed, sumvx, sumvy, sumvz, sumv2)
         sumv2 = sumv2*3*NPART
         S = 0
         PS = (sumv2-G*Temprqs)/Q
      ELSE
...
c     ---write input data
      ...
      G = 3*NPART
      PRINT *, ' ps ', PS, sumv2, G*Temprqs
      WRITE (6, 99006) Q, G, Temprqs
c
      PI = 4*ATAN(1.D0)
      RETURN
...
99006 FORMAT (' =====> Nose-Hoover thermostat <=========== ', /, 
     &        '  Q                               : ', f10.3, /, 
     &        '  g (number of degrees of freedom): ', i10, /, 
