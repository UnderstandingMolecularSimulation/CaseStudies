**==md_andersen.spg  processed by SPAG 4.52O  at 09:34 on 20 Jun 1996
      PROGRAM MD_ANDERSEN
...
c
c
c  Case Study 10: Lennard-Jones: Andersen thermostat
c
c__________________________________________________________________________
...
c     ---total energy of the system
      ...
      step = 0
      time = 0
      CALL SAMPLE(0, step, en, vir, enk, delt)
      nstep = INT(tmax/delt)
...
c        ---velocity Verlet
         CALL SOLVE(1, fx, fy, fz, enk, delt, iseed, temprsq)
         CALL FORCE(fx, fy, fz, enpot, vir)
         CALL SOLVE(2, fx, fy, fz, enk, delt, iseed, temprsq)
         time = time + delt
         en = enpot + enk
         step = step + 1
...
c           ---if system equilbrated sample averages:
     ...
     &          CALL SAMPLE(1, step, en, vir, enk, delt)
         END IF
...
c           ---write intermediate configuration to file
      ...
      CALL SAMPLE(2, step, en, vir, enk, delt)
